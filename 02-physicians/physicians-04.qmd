...
where $B(\alpha, \beta)$ is a normalization constant to ensure the PDF integrates to 1.
...
- ${n \choose s}$ and $B(\alpha, \beta)$ are just normalization constants, so we can ignore them
